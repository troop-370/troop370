...
<div class="panel" class:visible bind:offsetWidth={width}>
  <Tooltip
    text="{editor?.isActive('youtubeWidget', { playerPaused: false }) ? 'Pause' : 'Play'} video"
  >
    <Button
  ...
  <YoutubeVideoIdDialog
    ...
    handleSumbit={async (videoId) => {
      editor
        ...
        .focus()
        .selectParentNode()
        .updateAttributes('youtubeWidget', { videoId })
        .run();
    }}
  />
  ...
  <Tooltip
    text="{editor?.isActive('youtubeWidget', { showCaption: true })
      ? 'Hide'
      : 'Show'} video caption"
  >
    <Button
