<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    select: { month: number; day: number; year: number };
  }>();
  ...
  function adjustDay() {
    ...
    const maxDay = new Date(year, month, 0).getUTCDate();
    if (maxDay < day) {
  ...
  async function selectMonth(_month: number) {
    (
      monthElement?.querySelector(`[data-month='${_month}']`) as HTMLDivElement | undefined
    )?.focus();
    month = _month;
    adjustDay();
  }
  ...
  function cancel() {
    ...
    month = initialMonth ?? new Date().getUTCMonth() + 1;
    day = initialDay ?? new Date().getUTCDate();
    year = initialYear ?? new Date().getUTCFullYear();
  }
...
{#if open}
  ...
  >
    ...
    <div class="day" bind:this={dayElement} tabindex="-1">
      ...
      <ComboBoxItem disabled />
      <ComboBoxItem disabled />
      {#each [...Array(new Date(year, month, 0).getUTCDate()).keys()] as _, index}
        {@const _day = index + 1}
        <ComboBoxItem
          selected={day === _day}
