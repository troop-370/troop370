<script lang="ts">
  ...
  let am: 'AM' | 'PM' = new Date().getUTCHours() >= 12 ? 'PM' : 'AM';
  ...
  $: {
    ...
    tick().then(() => {
      ...
      const selectedMinuteElem = minuteElement?.querySelector('.selected') as
        | HTMLElement
        | undefined;
  ...
  async function selectMinute(_minute: number) {
    (
      minuteElement?.querySelector(`[data-minute='${_minute}']`) as HTMLDivElement | undefined
    )?.focus();
    minute = _minute;
  }
  ...
  function save() {
    ...
    if (am === 'AM' && hour === 12) {
      dispatch('select', { hour: 0, minute });
    } else if (am === 'PM' && hour === 12) {
      dispatch('select', { hour: 12, minute });
    } else {
  ...
  function cancel() {
    ...
    hour = initialHour ?? new Date().getUTCHours() + 1;
    minute = initialMinute ?? new Date().getUTCMinutes();
    am = (initialAm ?? new Date().getUTCHours() >= 12) ? 'PM' : 'AM';
  }
...
<style>
  ...
  .selector {
    ...
    -webkit-animation:
      menu-in var(--fds-control-normal-duration) var(--fds-control-fast-out-slow-in-easing),
