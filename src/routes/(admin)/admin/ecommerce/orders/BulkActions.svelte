<script lang="ts">
  ...
  export let tableData: z.infer<typeof orderEntrySchema>[];
  ...
  $: links = {
    ...
    windowName: `order-page-troop-370-`,
  };

  $: firstSelectedHref = `${links.href}/${tableData.find((doc) => doc.id === $selectedIds[0])?.id}${
    links.hrefSearch || ''
  }`;
