...
export const load = (async ({ fetch, parent, params, url, depends }) => {
  ...
  const { session, permissions, collectionConfig } = await parent();
  ...
  const sort = (() => {
    ...
    if (Object.keys(sort).length === 0) {
      const defaultSortKey =
        ...
        get(collectionConfig)?.settings.defaultSortBy ||
        'createdAt';
      const defaultSortKeyOrder = (() => {
        ...
        return get(collectionConfig)?.settings.defaultSortOrder.toLowerCase() === 'asc' ? 1 : -1;
      })();
...
function paramsToFilter(search: URLSearchParams) {
  ...
  for (const [key, value] of search.entries()) {
    ...
    if (!key.startsWith('__') && key !== '_search') {
